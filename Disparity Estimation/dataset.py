...
class KITTIDataset(object):
    ...
    def __getitem__(self, i):
        ...
        if self.downsample:
            img_l = rescale(
                img_l, 0.5, mode="reflect", anti_aliasing=True
            )
            img_r = rescale(
                img_r, 0.5, mode="reflect", anti_aliasing=True
            )
...
class PatchProvider(object):
    ...
    def iterate_batches(self, batch_size):
        ...
        #labels = np.ones((self._cache * batch_size,), dtype="float32")
        ...
        while True:
            ...
            with (self._lock):
                yield (
                    torch.Tensor(ref_batch[self.idxs]),
                    torch.Tensor(pos_batch[self.idxs]),
                    torch.Tensor(neg_batch[self.idxs]),
                    #torch.Tensor(labels[self.idxs]),  # Yield labels
                )
