...
def get_upsampling_weight(in_channels, out_channels, kernel_size):
    """Make a 2D bi-linear kernel suitable for upsampling"""
...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        enc2 = F.relu(self.conv2(enc1))
        enc2 = self.pool(enc2)

        enc3 = F.relu(self.conv3(enc2))
        enc3 = self.pool(enc3)

        enc4 = F.relu(self.conv4(enc3))
        enc4 = self.pool(enc4)
...
class Decoder(nn.Module):
    ...
    def forward(self, enc1, enc2, enc3, enc4):
        dec1 = F.relu(self.upconv1(enc4))
        dec1 = torch.cat((dec1, enc3), dim=1)
        dec1 = F.relu(self.conv1(dec1))

        dec2 = F.relu(self.upconv2(dec1))
        dec2 = torch.cat((dec2, enc2), dim=1)
        dec2 = F.relu(self.conv2(dec2))

        dec3 = F.relu(self.upconv3(dec2))
        dec3 = torch.cat((dec3, enc1), dim=1)
        dec3 = F.relu(self.conv3(dec3))
...
class PortraitFCN(FCN8s):
    def __init__(self, n_class=2, dropout=0.5):
        super(PortraitNet, self).__init__(n_class=n_class, dropout=dropout)
        super().__init__(n_class, dropout)
        self.encoder = Encoder()
        self.decoder = Decoder()

    def forward(self, x):
        enc1, enc2, enc3, enc4 = self.encoder(x)
        output = self.decoder(enc1, enc2, enc3, enc4)
        return output
...
class PortraitFCNPlus(PortraitFCN):
    def __init__(self, load_weights=False, dropout=0.5):
        ...
        self.name = "pfcnp"
        if load_weights:
            path_to_weights = "/path/to/portraitfcn_untrained.pth"
